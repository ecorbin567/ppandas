...
class QueryHelper:
    ...
    def query(self, bayes_net, query_vars, evidence_vars):
        ...
        if evidence_vars is not None:
            new_evidence_vars, need_to_expand = self.mapEvidenceVars(evidence_vars)
        if need_to_expand:
    ...
    def performExpandedQueries(self, bayes_net, query_vars, list_of_new_evidence_vars):
        df_res = None
        # evidenve1  |  evidence 2  |....  | P()
        # ----------- | ------------ |....  | ---
        df_evidence_probability = BayesNetHelper.query(
            bayes_net, list_of_new_evidence_vars[0].keys(), evidence_vars=None
        )
        for evidence_vars in list_of_new_evidence_vars:
            if df_res is None:
                ...
                df_new = BayesNetHelper.query(bayes_net, query_vars, evidence_vars)
                y = df_new.iloc[:, -1].values.astype(
                    float
                ) * self.get_probability_of_evidences(
                    df_evidence_probability, evidence_vars
                )
    ...
    def get_probability_of_evidences(self, df_evidence_probability, evidence_vars):
        i = None
