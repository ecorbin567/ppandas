...
class PDataFrame:
    ...
    def __init__(self, independent_vars, data, **kwargs):
        ...
        if isinstance(data, pd.DataFrame):
            ...
            if len(vars_not_in_dataframe) > 0:
                raise ValueError(
                    "Specified independent variables not found \
                in DataFrame: {} ".format(
                        str(vars_not_in_dataframe)[1:-1]
                    )
                )
            self.dependent_vars = set(
                [var for var in self.vars if var not in self.independent_vars]
            )
            self.bayes_net = BayesNetHelper.single_bayes_net(
                data, self.independent_vars, self.dependent_vars
            )
            self.atomic = True

        elif isinstance(data, BayesianNetwork):
    ...
    def from_populational_data(cls, independent_vars, data, num_of_records):
        ...
        dependent_vars = set([var for var in variables if var not in independent_vars])
    ...
    def pjoin(self, df2, mismatches=None):
        ...
        if len(overlap) > 0:
            raise ValueError(
                "This join can not be performed since independent\
                            ...
                            switching reference distribution.".format(
                    str(overlap)[1:-1]
                )
            )
        ...
        new_dependent_vars = (
            self.dependent_vars | df2.dependent_vars - self.independent_vars
        )
        new_independent_vars = (
            self.independent_vars | df2.independent_vars - new_dependent_vars
        )
        ...
        if mismatches:
            for mNode, mType in mismatches.items():
                if mNode not in self.independent_vars:
                    raise ValueError(
                        "Only mismatches across independent\
                    ...
                    raise ValueError(
                        "mismatch type {} is currently not \
                        supported".format(
                            mType
                        )
                    )
        ...
        pdataframe_joined = PDataFrame(
            new_independent_vars,
            final_bayes,
            num_of_records=new_num_of_records,
    ...
    def query(self, query_vars, evidence_vars=None, entries="reference"):
        if not self.atomic:
            ...
            res = BayesNetHelper.query(self.bayes_net, query_vars, evidence_vars)
        return res
    ...
    def map_query(self, query_vars, evidence_vars=None, entries="reference"):
        if not self.atomic:
            ...
            res = BayesNetHelper.map_query(self.bayes_net, query_vars, evidence_vars)
        return res
    ...
    def predict(self, test_df, query_var, evidence_vars, query_var_state=None):
        ...
        for index, row in test_df.iterrows():
            ...
            test_df.loc[index, query_var[0]] = self.parseQuery(
                query_var[0],
                test_row_query_df,
    ...
    def parseQuery(
        self, query_var, query_result_df, probability_column_name, query_var_state
    ):
        # Find query_var state with highest Probability
        probability_column_name = "Probability(" + str(query_var) + ")"
        if not query_var_state:
            query_var_state_row_index = query_result_df[
                probability_column_name
            ].idxmax()
            ...
            return query_result_df.loc[
                query_result_df[query_var] == query_var_state, probability_column_name
            ].values[0]
